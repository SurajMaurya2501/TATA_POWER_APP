...
class _SafetySummaryState extends State<SafetySummary> {
  ...
  Future<void> getRowsForFutureBuilder() async{
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('SafetyChecklistTable2')
    .doc('${widget.depoName}').collection('userId').get();

    List<dynamic> userIdList = querySnapshot.docs.map((e) => e.id).toList();

    for(int i = 0 ; i< userIdList.length ; i++){

      QuerySnapshot userEntryDate = await FirebaseFirestore.instance.collection('SafetyChecklistTable2')
          .doc('${widget.depoName}')
      .collection('userId')
          .doc(userIdList[i])
          .collection('date').get();
  ...
  Future<void> _generatePDF(String user_id,String date,int decision) async {
    ...
    final white_background = pw.MemoryImage(
      (await rootBundle.load('assets/white_background2.png')).buffer.asUint8List(),
    );
    ...
    Map<String,dynamic> safetyMapData = safetyFieldDocSanpshot.data() as Map<String,dynamic>;
    ...
    List<List<dynamic>> fieldData = [  ['Installation Date', '$date1'] , ['Enegization Date' , '$date2'] , ['On Boarding Date' , '$date3']
      , ['TPNo : ', '${safetyMapData['TPNo']}'],['Rev :' , '${safetyMapData['Rev']}'],
      ['Bus Depot Location :', '${safetyMapData['DepotLocation']}'],
      ['Address :' ,'${safetyMapData['Address']}'], ['Contact no / Mail Id :' ,'${safetyMapData['ContactNo']}' ],
      ['Latitude & Longitude :' , '${safetyMapData['Latitude']}'],
      ['State :','${safetyMapData['State']}'] , ['Charger Type : ','${safetyMapData['ChargerType']}'] ,
    ...
    if(docData.isNotEmpty){
      ...
      for(Map<String,dynamic> mapData in userData){
        ...
        String images_Path ='gs://tp-zap-solz.appspot.com/SafetyChecklist/Bengaluru/${widget.depoName}/$user_id/${mapData['srNo']}';
        ListResult result = await FirebaseStorage.instance.ref().child(images_Path).listAll();
    ...
    pdf.addPage(
      ...
      pageFormat: const PdfPageFormat
        (1300, 900,marginLeft: 70,marginRight: 70,marginBottom: 80,marginTop: 40),
        ...
        return pw.Container(
            ...
            child: pw.Column(children: [
              pw.Row(
                  ...
                  children: [
                    pw.Text('Safety Report', textScaleFactor: 2,
                        style: const pw.TextStyle(
